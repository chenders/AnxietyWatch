...
- **Never-requested HealthKit authorization is not "no data":** until the read-authorization sheet has been presented (`HKAuthorizationRequestStatus.shouldRequest`), every read errors with code 5 (authorizationNotDetermined), which `HealthKitManager`'s nil-coercion makes indistinguishable from an empty store. Any writer of HealthKit-derived values (`aggregateDay`, backfill) must gate on `authorizationNeedsRequest()` and skip **the HealthKit-derived fields only** — CPAP/barometric/sensor-derived/precedence/dataQuality stitching reads SwiftData, not HealthKit, and must keep running (see `aggregateDay`'s reduced pass). Without the gate, aggregation overwrites real (e.g. server-restored) values with nils, and the backfill marks itself done having read nothing. The gate **fails closed**: `.unknown`, a status-API error, or unavailable HealthKit all count as "needs request" — skipping one pass is recoverable, shredding data is not. A bundle-ID rename silently re-enters this state on an install full of data; authorization is requested at launch by `HealthDataCoordinator.requestAuthorizationIfNeeded()`, with the Settings → Apple Health button as the manual re-request path.
- **A day's HealthKit data keeps arriving after the day ends:** the Watch delivers overnight sleep/respiratory rate after wake, and Apple writes a day's final resting-HR sample near midnight. Refresh paths must re-aggregate the trailing days via `SnapshotAggregator.aggregateRecentDays(endingAt:)` (`recentAggregationLookbackDays`), never just `aggregateDay(.now)` — a today-only refresh freezes every late-landing field out of yesterday's snapshot forever. That freeze ran silently for three months (Apr–Jul 2026) and only surfaced when a restore round-tripped the server's frozen shells back onto a device.
