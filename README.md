...
- **Vision OCR for prescription labels** — `PrescriptionLabelScanner` extracts Rx number, medication name, and dosage from photographed pill bottles using regex patterns against `VNRecognizeTextRequest` output.
...
- **Extracted view model pattern** — `DashboardViewModel` shows how to pull business logic out of SwiftUI views into testable `@Observable` classes, with a thorough test suite covering sample loading, baseline computation, and trend calculation.
