...
struct SyncServiceTests {
    ...
    // MARK: - deactivateStaleMedications

    @Test("Deactivates medication whose most recent prescription is older than staleness limit")
    func deactivateStale() throws {
        let container = try makeContainer()
        let context = ModelContext(container)

        let med = MedicationDefinition(name: "Alprazolam", defaultDoseMg: 0.5)
        context.insert(med)
        ...
        let rx = Prescription(
            ...
            doseMg: 0.5,
            quantity: 30,
            dateFilled: Calendar.current.date(byAdding: .day, value: -90, to: .now)!,
            medication: med
        )
        context.insert(rx)
        try context.save()
        ...
        #expect(med.isActive == false)
    }

    @Test("Keeps medication active when it has a recent prescription")
    func keepRecentActive() throws {
        let container = try makeContainer()
        let context = ModelContext(container)

        let med = MedicationDefinition(name: "Lorazepam", defaultDoseMg: 1.0)
        context.insert(med)
        ...
        let rx = Prescription(
            ...
            doseMg: 1.0,
            quantity: 30,
            dateFilled: Calendar.current.date(byAdding: .day, value: -10, to: .now)!,
            medication: med
        )
        context.insert(rx)
        try context.save()
    ...
    @Test("Leaves manually-added medication with no prescriptions unchanged")
    func manualMedUnchanged() throws {
        let container = try makeContainer()
        let context = ModelContext(container)

        let med = MedicationDefinition(name: "Vitamin D", defaultDoseMg: 1000)
        context.insert(med)
        try context.save()
