...
- **Never-requested authorization is not "no data".** Until the read-authorization sheet has been presented (`HKAuthorizationRequestStatus.shouldRequest`), every read errors with code 5 (authorizationNotDetermined), which the query layer coerces to nil. Any code that *writes HealthKit-derived values* (`SnapshotAggregator.aggregateDay`, backfill) must gate on `authorizationNeedsRequest()` and skip the HealthKit-derived fields only — non-HealthKit stitching (CPAP, barometric, sensor-derived, precedence, dataQuality) reads SwiftData and must keep running (see `aggregateDay`'s reduced pass). The gate fails closed (`.unknown`/status error/unavailable → "needs request"): skipping one pass is recoverable, writing nils over real data is not. A bundle-ID change silently re-enters this state on an install full of data. Flag any new HealthKit-derived writer that lacks the gate, and any `authorizationNeedsRequest` implementation that fails open.
- **A day's HealthKit data keeps arriving after the day ends.** The Watch syncs overnight sleep/respiratory rate after wake; Apple writes a day's final resting-HR sample near midnight. Refresh paths must re-aggregate the trailing days (`SnapshotAggregator.aggregateRecentDays(endingAt:)` / `recentAggregationLookbackDays`), never just `aggregateDay(.now)` — a today-only refresh silently freezes every late-landing field out of yesterday's snapshot forever. Flag any new call site that aggregates only `.now`.
