...
- `name`: user-entered preferred name, used in analysis reports for readability (e.g., "Jane's HRV trend..." instead of "The patient's HRV trend...")
...
If `patient_profile.name` is set, the analysis should use the patient's name throughout the response for readability (e.g., "Jane's HRV was elevated" rather than "The patient's HRV was elevated").
