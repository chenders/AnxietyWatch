...
12. **Synergy detection is the UNION of two rules** (amended mid-execution, 2026-07-12): a 24h dose-pairing horizon OR §14.1 monitoring-window overlap. The horizon-only variant misses methadone@t0 + benzo@t0+50h (the 50h gap exceeds the 24h pairing horizon; the overlap leg catches it because the benzo dose falls inside methadone's 72h monitoring window — union preserves t0+134h vs. t0+72h, a 62h difference). The overlap-only variant misses benzo@t0 + opioid@t0+23h50m (the benzo's 12h window closed at t0+12h, so the windows never overlap; the horizon leg catches the 23h50m pairing). Constants: `CNSMonitoringConstants.synergyPairingHorizon`/`synergyWindowExtension`/`synergyWindowFloor`.
13. **Background execution limits (final-review fix, 2026-07-12).** Phase 2's tick loop is a 1 Hz `Task.sleep`-based loop on the main actor (`CNSMonitoringCoordinator.startTickLoopIfNeeded`) — it has NO background-execution guarantee. iOS can suspend the app seconds after backgrounding (e.g. the phone's screen locks overnight), at which point the loop simply stops running: no more sensor polling, no more device-loss detection, no more tier updates, and critically, no more of the local notifications §7/§14.2's "never a silent stop" rule depends on — all of that logic only runs INSIDE a tick. A silent suspension while monitoring is active (the EMAY-battery-dies-at-3am scenario) would otherwise fail exactly the way this feature exists to prevent: silently. The mitigation is a **dead-man's-switch watchdog**: every persist-cadence tick (re)schedules a LOCAL notification `CNSMonitoringConstants.deadMansSwitchInterval` (90s) out, cancelling/replacing the previous one; if the tick loop dies for any reason, nothing reschedules it again and it fires on time, telling the user monitoring may have stopped and to reopen the app. This bounds the worst-case silent gap to ~90s (plus up to one persist-cadence interval, 10s) — it is a bound on DETECTION latency, not a fix for the underlying suspension; the app does not keep monitoring in the background. Robust background monitoring that survives suspension outright — BLE-event-driven device-loss detection (CoreBluetooth state restoration, which `PolarHRMService` and `EMAYRealtimeService` already use for reconnect but not yet for the CNS pipeline itself) and **Critical Alerts** (§14.5, so the watchdog/device-loss notification can bypass silent/Focus at a system-controlled volume even while locked) — is explicitly Phase 3 scope, not Phase 2.
...
- `CNSDepressantClassifier`'s name-based word lists (benzodiazepine/Z-drug and opioid generics + brands) are a curated set of common US brand names, not an exhaustive pharmacopoeia — international brand names, less-common generics, and combination products are not guaranteed to match; clinical review should assess coverage gaps before this classifier is relied on as the sole detection path (the stored `MedicationDefinition.cnsDepressantClass` picker is the user-correctable source of truth precisely because the classifier is a default, not a guarantee).
