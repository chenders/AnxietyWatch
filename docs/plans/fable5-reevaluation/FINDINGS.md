...
**Disposition:** fixed (#179). Chose `CPAPSession.ahi: Double?` (nil = unknown/EDF-only night, distinct from a measured 0.0) over a numeric sentinel — 0 is a valid AHI, so a sentinel could silently corrupt averages. SwiftData automatic lightweight migration handles the Double→Double? widening (no VersionedSchema in the app target). RestoreFromServer drops the ahi cast-guard so null-AHI rows import with `ahi == nil`. All ~26 consumers updated to exclude unknown nights from aggregation (`compactMap(\.ahi)`), render "—", and rank nil last in tie-breaks. Two review-surfaced propagation bugs fixed in-PR: (a) `LastNightHeadline.compose` coerced `(ahi ?? 0) >= 5`, letting an unscored night certify "Solid night" — now gated by `ahiMeasured` mirroring the F-024 `effMeasured` rule; (b) the `max(by:)` CPAP dedup tie-break mapped nil→-1, which (because the comparator picks the lowest AHI) made a nil-AHI duplicate WIN over a scored one — Copilot caught this; fixed to nil→`.infinity` so a scored session always wins. Regression tests: `RestoreFromServerTests`, `ReportGeneratorTests.cpapAverageExcludesUnknownAHI`, `LastNightHeadlineTests` (unknown-AHI caps at OK / not a breach), `DashboardViewModelTests.cpapSessionTieBreakPrefersScoredOverNil`.
