...
# Concrete simulator for tests (dynamically finds first available iPhone)
IOS_TEST_DEST := $(shell xcrun simctl list devices available -j 2>/dev/null | python3 -c "import sys,json; devs=json.load(sys.stdin)['devices']; print(next(('platform=iOS Simulator,name='+d['name'] for rt in devs for d in devs[rt] if 'iPhone' in d['name'] and d['isAvailable']),  'platform=iOS Simulator,name=iPhone 16 Pro'))" 2>/dev/null)
