...
- **Incremental-sync cursor advanced to `.now` after I/O**: Any pattern of the shape `let payload = build(since: lastSyncDate); await POST(payload); lastSyncDate = .now` has a race: a record created between `build` and the `.now` assignment is not in the payload (created after the fetch) AND has a timestamp less than the new cursor (so the next sync's `since` filter skips it). The cursor must be captured BEFORE the payload build, passed to the export as `end:`, and assigned to that captured value on success. Especially dangerous in drain loops where each iteration opens a new window. Flag any sync-like code that reads `lastSyncDate`/equivalent, performs I/O, then assigns the cursor to `.now` or `Date()` post-I/O instead of a pre-captured upper bound.
- **Cursor advanced past tables that weren't exported**: A corollary of the previous rule. If a drain loop conditionally skips some tables on subsequent iterations (e.g., `bulkOnly: true` that omits small-volume tables, or a "headers-only" mode), advancing the cursor on those iterations is equivalent to the original race — records in the omitted tables were never sent, but their timestamps now fall under the new cursor. Flag any cursor advance that isn't conditional on the payload actually carrying the relevant table. The pattern is "advance the cursor for table X only on iterations where the payload exported table X."
- **Conditional-skip optimizations breaking invariants the unconditional version preserved**: Whenever a diff adds a branch that skips work the original code did unconditionally (a `bulkOnly`/`fast`/`cached`/`skipOnRetry` flag, lazy short-circuit, "if expensive" gate), audit every downstream state mutation that was paired with that work in the original code. Cursor advances, flag flips, counter increments, idempotency markers, log/audit writes — any state whose correctness assumed the skipped work happened must become conditional too, or its invariant silently breaks. The PR-141 sync drain is the canonical case: round-2 added `bulkOnly: roundTrips > 0` to skip small-volume exports, but left `lastSyncDate = cursorUpperBound` unchanged, reintroducing the round-1 race in a smaller window. When reviewing any new conditional-skip optimization, identify what the skipped work was paired with and verify each pairing was carried through the conditional, not just the work itself.
...
- **Compound `#Predicate` with captured non-primitive locals**: `#Predicate<T> { $0.fk == capturedUUID && $0.col == capturedString }` can hang the main thread in SwiftData's SQL ORDER BY generation on iOS 26 (`+[_NSPredicateUtilities _predicateEnforceRestrictionsOnSelector:…]`, scene-update watchdog `0x8BADF00D`). Flag any compound `#Predicate` clause where one term references a captured local of a non-primitive type (UUID, String captured via `let`) and suggest splitting into a single-clause primary predicate plus an in-memory filter for the secondary condition. Single-clause predicates do not hit this path.
- **`NavigationLink` destinations that contain `@Query`**: Closure-based `NavigationLink { DestinationView(...) } label: { … }` rebuilds the destination struct on every parent body re-render. SwiftUI's default memberwise comparison cannot dedupe when the struct contains `@Query` (or any property wrapper with internal observation state), so the NavigationStack push animation restarts every frame, cascading into a ~30 Hz render loop and eventually a `CA::Layer::layout_is_active` use-after-free on iOS 26. Flag any new such destination missing either (a) `Equatable` conformance on identity props only or (b) `.equatable()` at the call site (`DetailView(id: x).equatable()`).
- **`chartYScale(domain:)` paired with `.nan` y-values**: Swift Charts on iOS 26 has a pathologically slow layout when a `LineMark` y-value can be `.nan` (the gap-drawing pattern) AND the axis is clamped via `.chartYScale(domain: 0...upper)`. Flag any new `.chartYScale(domain:)` whose data feeder can emit `.nan` and suggest pre-clipping at the data layer (`point.value.map { min($0, upper) } ?? .nan`) so the chart auto-scales to the clipped data instead.
