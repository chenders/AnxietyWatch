...
struct DashboardAlert: Identifiable, Sendable {
    ...
    enum Category: Sendable { case autonomic, sleep, environment }
    ...
    /// Signed z-score against personal baseline; 0 for non-statistical alerts.
    let zScore: Double
...
/// category plus a "+N related" chip.
nonisolated enum AlertsDeduper {
    static func collapse(alerts: [DashboardAlert]) -> [DashboardAlert] {
        ...
        let order: [DashboardAlert.Category] = [.autonomic, .sleep, .environment]
