...
public actor OuraAPIClient {
    ...
    // Internal name `serverURL` avoids shadowing the actor's own `baseURL`
    // (Oura's API host) with the caller's *sync server* URL; the external label
    // stays `baseURL` so call sites are unchanged.
    public func postTokenToServer(baseURL serverURL: String, apiKey: String, token: OuraTokenStore.Token) async throws {
        ...
        guard let url = URL(string: serverURL)?.appendingPathComponent("api/oura/auth") else {
            // No request was sent — a malformed local URL, not an HTTP status.
        ...
        let payload = AuthPayload(
            ...
            expiresAt: formatter.string(from: token.expiresAt)
        )
    ...
    public func fetchCardiovascularAge(startDate: String, endDate: String) async throws -> [OuraCardiovascularAgeData] {
        return try await fetch(
            endpoint: "daily_cardiovascular_age", startDate: startDate, endDate: endDate,
            responseType: OuraCardiovascularAgeResponse.self
        ).data
    }
